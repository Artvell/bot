...
async def rotate_btn(callback_query: types.CallbackQuery,state):
    ...
    if callback_query.data == "Предыдущий":
        if (filter_info is not None) and (filter_info != {}):
            product,caption,prod_id = product_message_by_category(filter_info["parameter"],filter_info["flag"],prev_param_id,False,True)
        else:
            product,caption,prod_id = create_product_message(prev_id, False,is_superuser=True)
    else:
        if (filter_info is not None) and (filter_info) != {}:
            product,caption,prod_id = product_message_by_category(filter_info["parameter"],filter_info["flag"],prev_param_id,True,True)
        else:
            product,caption,prod_id = create_product_message(prev_id, True, is_superuser=True)
    if product is not False:
        ...
        await bot.edit_message_media(
            ...
            media = new_photo,
            reply_markup=product_keyboard(flag,post.is_available,post.is_visible)
        )
        if filter_info is not None and filter_info != {}:
